...
const getSections = async () => {
    return await db_1.db.select().from(schema_1.complianceSections);
};
...
const createSection = async (user, data) => {
    ...
    const existing = await db_1.db
        ...
        .where((0, drizzle_orm_1.eq)(schema_1.complianceSections.code, data.code))
        .limit(1);
    if (existing.length) {
        throw new Error("Compliance section with this code already exists");
    }
    const newSection = await db_1.db
        .insert(schema_1.complianceSections)
        .values({
        code: data.code,
        title: data.title,
