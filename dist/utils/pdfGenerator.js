...
const registerFonts = (doc) => {
    ...
    const fontPaths = [
        path_1.default.join(process.cwd(), "fonts", "DejaVuSans.ttf"),
        path_1.default.join(process.cwd(), "fonts", "DejaVuSans-Bold.ttf"),
        "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
        "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",
    ];
    let fontsRegistered = false;
    // Try DejaVu Sans
    if (fs_1.default.existsSync(fontPaths[0]) && fs_1.default.existsSync(fontPaths[1])) {
        doc.registerFont("DejaVuSans", fontPaths[0]);
        doc.registerFont("DejaVuSans-Bold", fontPaths[1]);
        fontsRegistered = true;
    }
    else if (fs_1.default.existsSync(fontPaths[2]) && fs_1.default.existsSync(fontPaths[3])) {
 ...
 */
const ensureSpace = (ctx, requiredHeight) => {
    const { doc } = ctx;
...
// LOGO DRAWING
// ============================================================================
const drawLogo = (doc, x, y, size = 45) => {
    try {
...
const drawHeader = (ctx) => {
    ...
    doc
        ...
        .font(FONTS.bold)
        .fillColor(COLORS.primary)
        .text("Corporate Affairs Commission", PAGE.MARGIN + logoSize + 15, headerTop + 2, {
        width: 300,
    });
    doc
        .fontSize(8)
        ...
        .text("Compliance Management System", PAGE.MARGIN + logoSize + 15, headerTop + 24);
...
const drawFooter = (ctx) => {
    ...
    doc.text("© 2026 Corporate Affairs Commission • Official Document", PAGE.MARGIN, footerY + 6, { width: 200, align: "left", lineBreak: false });
...
const drawTable = (ctx, columns, rows, options = {}) => {
    const { doc } = ctx;
    const { headerColor = COLORS.tableHeader, fontSize = 8, cellPadding = 6, rowPadding = 3, headerHeight = 22, } = options;
    ...
    columns.forEach((col, colIdx) => {
        const x = startX + columns.slice(0, colIdx).reduce((sum, c) => sum + c.width, 0);
        doc
            .fontSize(fontSize)
            .font(FONTS.bold)
            .fillColor(COLORS.white)
            .text(col.header.toUpperCase(), x + cellPadding, currentY + (headerHeight - fontSize - 2) / 2, {
...
// ============================================================================
const drawCaseBlock = (ctx, caseData, _index) => {
    const { doc } = ctx;
    ...
    doc
        ...
        .font(FONTS.bold)
        .fillColor(statusColor)
        .text(caseData.status.toUpperCase().replace(/_/g, " "), badgeX + 4, startY + 9, { width: badgeWidth - 8, align: "center", lineBreak: false });
    doc.restore();
...
const generatePDF = (options) => {
    ...
    doc
        ...
        .text(`Report Type: ${options.reportType === "overall" ? "Overall Compliance" : "Cases Report"}`, { align: "center" })
        .moveDown(1);
    // Separator
    doc
    ...
    doc
        ...
        "Unauthorized distribution or reproduction of this document is strictly prohibited.", { align: "center", width: PAGE.CONTENT_WIDTH });
    doc.end();
    return stream;
};
